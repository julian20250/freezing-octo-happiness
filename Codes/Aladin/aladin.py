...
for x in xrange(0, 2):
  for y in readable[head[x]]:
    unicodedata.normalize('NFKD', y).encode('ascii','ignore')
for x in xrange(2, 4):
  for y in readable[head[x]]:
      road = y.split(" ")
...
def objects():
  f = open(location+"/Raw/class", "r")
  you=[]
  count=1
  for x in f.readlines():
    if count != 1 and count != 233:
      you.append(x)
    else:
      pass
    count+=1
  you = [x.split(" ") for x in you]
  for x in you:
    while True:
      try:
	x.remove("")
...
def main():
  ...
  def indexer_real(v): #B=readable[head[9]], V=readable[head[10]]
	...
	if v.get()==1:
	  plt.scatter(ball[maxim[x]], inst[x], c=colors[cool.index(readable[head[1]][maxim[x]])],
	      marker=markers[cool.index(readable[head[1]][maxim[x]])], label = objecter[objecter.index(readable[head[1]][maxim[x]])+2])
	elif v.get()==2:
	  plt.scatter(ball[maxim[x]], inst[x], c=colors[cool.index(readable[head[1]][maxim[x]])],
    ...
    if v.get()==1:
      plt.legend(bbox_to_anchor=(1, .9), fontsize=8,
  ...
  def indexer(wr): #B=readable[head[9]], V=readable[head[10]]
	...
	if s==0:
	  menu.add_command(label="Show All Objects", command=lambda: indexer_real(wr))
	  s+=1
  ...
  def plot2d(v): 	#Plot the Location
    ev = Toplevel()
	...
	if dawn.count(readable[head[1]][x]) == 1:
	  if v.get()==1:
	    plt.scatter(readable[head[2]][x], readable[head[3]][x], c=colors[cool.index(readable[head[1]][x])],
		    marker=markers[cool.index(readable[head[1]][x])], label = objecter[objecter.index(readable[head[1]][x])+2])
  ...
  v=IntVar()
  v.set(1)
  Label(burn, text="Complete Object Classification").grid()
  Radiobutton(burn, text="Enabled", variable=v, value=1).grid()
  Radiobutton(burn, text="Disabled", variable=v, value=2).grid()
  l0 = Label(burn, text="")#\n
  ...
  setupmenu.add_command(label="Object Classification in SIMBAD", command=lambda: os.system("gedit "+location+"/Raw/class"))
  plotmenu = Menu(menu)
  menu.add_cascade(label="Plots", menu=plotmenu)
  plotmenu.add_command(label="Position (RA, DEC)", command=lambda: plot2d(v))
  plotmenu.add_command(label="Index B-V", command=lambda: indexer(v))
