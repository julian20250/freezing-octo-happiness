...
def get_data(rai):
    ...
    br.open('http://www.rssd.esa.int/hipparcos_scripts/hipMultiSearch1.pl')
    br.select_form(nr=0)
    br.form['Catalogues'] = [catalogues[v.get()]]
    if b["0"].get()==1:
    ...
    br.submit()
    pon=br.response().read()
    print pon[pon.index('color="#ff0000">')+16:pon.index("request.")+8]
    with open("table.txt", "w") as f:
        f.write(pon[pon.index("your request.\n</font></b>")+25:pon.index("</body>\n</html>")])
        f.close()
    flag.append("table.txt")
...
while True:
    ...
    frame.mainloop()
    data = ascii.read(flag[0])
    par = data["col12"]
    M=[]
    ind = []
    m= data["col"+str(sec.get())]
    for x,y,z in zip(par,m,data["col"+str(fir.get())]):
        try:
            M.append(float(y)+5-5*np.log(1./float(x)))
            ind.append(float(z))
        except:
            pass
    print "There were %i valid entries"%len(M)
    ax= plt.subplot()
